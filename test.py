...
def random_cmos_opcode() -> int:
    return random.choice([  ])


def random_testcase(opcode):
...
class CargoEmulator(Emulator):
    ...
    def __init__(self, name, bin_name="test"):
        super().__init__(name)
    ...
    def test(self, test_case):
        cycles = []
        cmd = ["cargo", "run", "-q", "--bin", self.bin_name, "--"] + [f'{c:02x}' for c in test_case]
...
mre_mos6502 = CargoEmulator("mre_mos6502", bin_name="mre_mos6502")
mre_cmos = CargoEmulator("mre_mos6502", bin_name="cmos")
w65c02s = CargoEmulator("w65c02s", bin_name="w65c02s")
rubbermallet = MakeEmulator("rubbermallet")
gianlucag = MakeEmulator("gianlucag")


all_emulators = [gianlucag, mre_mos6502, w65c02s, rubbermallet]
all_cmos_emulators = [mre_cmos, w65c02s]

def bad_bcd(result):
    opcode = int(result[1].data_bus, 0)

    if opcode not in [0x69, 0x65, 0x75, 0x6d, 0x7d, 0x79, 0x61, 0x71, 0xe9, 0xe5, 0xf5, 0xed, 0xfd, 0xf9, 0xe1, 0xf1]:
        # the opcode is not ADC or SBC, so this is not an instance of bad BCD
        return False

    accumulator = int(result[0].internal_state()[0], 0)
    if accumulator > 0x90:
        return True
    if accumulator & 0x0f > 0x09:
        return True

    addend = int(result[-2].data_bus, 0)
    if addend > 0x90:
        return True
    if addend & 0x0f > 0x09:
        return True
...
def test(a, b):
    ...
    if a[1][0].internal_state() != b[1][0].internal_state():
        print(f"The initial states differ between {a_name} and {b_name}")
        print(str(a[1][0].internal_state()))
        print(str(b[1][0].internal_state()))
        failed = True

    if bad_bcd(b[1]):
        # Trying to do BCD arithmetic on duff values isn't going to work.
        return failed
...
for emu in all_emulators + all_cmos_emulators:
    emu.init()

def run_test(test_case, emus=all_emulators):
    print(" ".join([f'{c:02x}' for c in test_case]))
    results = list([(emu, emu.test(test_case)) for emu in emus])
...
while True:
    tc = random_testcase(random_opcode)
    if run_test(tc, all_emulators):
        print(f"A new regression test: {tc}")
        sys.exit(0)

    tc = random_testcase(random_cmos_opcode)
    if run_test(tc, all_cmos_emulators):
        print(f"A new regression test: {tc}")
        sys.exit(0)
